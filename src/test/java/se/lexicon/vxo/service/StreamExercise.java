...
public class StreamExercise {
    ...
    public void task1() {
        ...
        //integers.stream().forEach(System.out::println);//using method reference
        integers.stream().forEach(integer -> System.out.println());//using lambda expression
    ...
    public void task10() {
        ...
        optional = people.stream().filter(person ->person.getPersonId()==personId).map(person -> person.getDateOfBirth())
                .map(d->d.getDayOfWeek()+" "+d.getDayOfMonth()+" "+d.getMonth()+" "+d.getYear()).findFirst();
    ...
    public void task12() {
        ...
        String[] result = null;
        result = people.stream().filter(person -> new StringBuilder(person.getFirstName()).reverse().toString().equalsIgnoreCase(person.getFirstName()))
